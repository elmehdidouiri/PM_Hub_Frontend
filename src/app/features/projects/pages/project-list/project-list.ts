...
export class ProjectList implements OnInit, AfterViewInit {
  ...
  onExport(): void {
    ...
    this.notificationService.showInfo('Generating Excel export... please wait.');
    ...
    this.projectService.exportProjects(filters).subscribe({
      next: (blob) => {
        if (blob) {
          ...
          link.href = url;
          link.download = `PMHUB_Projects_Export_${new Date().toISOString().slice(0, 10)}.xlsx`;
          link.click();
      ...
      error: (error) => {
        this.notificationService.showError(this.extractErrorMessage(error, 'Unable to export projects.'));
      },
    });
  }
  ...
  private getStatusLabel(project: ProjectSummaryDto): string {
    ...
    return this.statusLabels[value as ProjectStatus] || `Status ${value}`;
  }
  ...
  private getPhaseLabel(project: ProjectSummaryDto): string {
    ...
    return this.phaseLabels[value as ProjectPhase] || `Phase ${value}`;
  }
